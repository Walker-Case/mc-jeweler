...
public class SpectralSpiderSummonEffect implements ISummonEffect {
    @Override
    ...
    public ResourceLocation effectID() {
        return new ResourceLocation(JewelerMain.MODID, "spectral_spider_summon");
    }
    ...
    public void playSummonSounds(LivingEntity entity) {
        entity.playSound(SoundEvents.SPIDER_AMBIENT, 1, 1);
    ...
    public LivingEntity createEntity(Level level, Player player, ItemStack stack) {
        ...
        SpectralSpider spider = new SpectralSpider(JewelerEntities.SPECTRAL_SPIDER.get(), level);
        spider.tame(player);
        spider.setPos(player.getX(), player.getY() + 1.5d, player.getZ());

        spider.setCustomName(JewelerMain.PLATFORM_UTIL.getLiteralComponent(player.getDisplayName().getString() + "'s Lvl." + effLevel + " ")
                .append(JewelerMain.PLATFORM_UTIL.getTranslatedComponent("literal.spider")));
        return spider;
    }
